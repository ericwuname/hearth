# codex-rust v22 全链路测试手册（v21 实测修正版）

> 给测试窗口。不需要懂 Rust、不需要懂项目架构。
> 每步都有：复制粘贴的命令 → 期望看到什么 → 看到不对怎么办。
> 总耗时约 60 分钟（v22 含 T6 基准 20×1 + T7 应力 8×3 + gemini 冒烟）（含机器跑分时间）。
...
```bash
# 先杀掉旧的服务（如果有的话）
fuser -k 3000/tcp 2>/dev/null; pkill -f "target/release/service" 2>/dev/null; sleep 2

# 启动服务（后台运行，日志写文件）
mkdir -p ~/codex_work/logs
cd ~/codex_work
nohup ./target/release/service > ~/service.log 2>&1 &
ln -sf ~/service.log ~/codex_work/logs/service.log 2>/dev/null
echo "服务启动中，等待 30 秒..."
sleep 30
```

注意：使用预编译 release 二进制（`cargo build --release` 已由开发者完成）。
...
|---|---|
| 返回 `OK` 或 `200` | ✅ 通过 |
| 无响应 / `connection refused` | ❌ 等 30 秒重试。还不行 → 看日志 `tail -50 ~/service.log` |
...
# 将 record_tool_exchange 调用整行替换为禁用字面量（wiring 用 contains 子串检查，
# 注释法会让子串仍命中 → 假绿，必须整体替换才触发真红）
sed -i 's/self\.record_tool_exchange();/self.__disabled_tool_exchange();/' crates/agent-core/src/loop.rs
```
...
```bash
fuser -k 3000/tcp 2>/dev/null; pkill -f "target/release/service" 2>/dev/null; sleep 3
cd ~/codex_work
nohup ./target/release/service > ~/service.log 2>&1 &
ln -sf ~/service.log ~/codex_work/logs/service.log 2>/dev/null
sleep 30
...
```bash
BENCH_PROVIDER=zhipu python3 bench/runner.py run T02-change-timeout --run 1
```
...
```bash
# 数下 PASS 了几个（按本次输出文件计数，避免历史文件重复）
BENCH_OUT=v22-baseline.jsonl
grep -c '"success": true' bench/results/raw/$BENCH_OUT 2>/dev/null
# 或直接看 runner 最后的 SUMMARY 行
```
...
```bash
cd ~/codex_work
VM_PW=123456 CODEX_VM_PW=123456 STRESS_PROVIDER=deepseek STRESS_RUNS=3 python3 bench/stress_v15.py
```
