    ...
    fn description(&self) -> ToolDescription {
        ToolDescription {
            ...
            description: "Find files matching a glob pattern.\n\
                 何时用: 已知文件名/扩展名要找位置——\"**/*.rs\"；按扩展名确认项目技术栈（依据真实文件，不猜）。\n\
                 何时不用: 找文件内容（用 grep）；文件名已知（直接 read）。\n\
                 示例: glob(\"**/*.go\") 列出全部 Go 文件；glob(\"src/*.ts\") 只列 src 顶层。\n\
                 边界: 返回路径列表（无内容、无行号）；无匹配返回 \"no matches\"=该项目没有此类文件——\n\
                 换技术栈假设，不要重复同款 pattern。\n\
                 错误解读: \"no matches\"=零命中（修正 pattern 或承认文件不存在）；\"path traversal denied\"=pattern 含 ..。"
                .into(),
